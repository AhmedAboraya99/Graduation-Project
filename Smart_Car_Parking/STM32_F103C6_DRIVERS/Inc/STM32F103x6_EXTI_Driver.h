#ifndef _STM32F103X6_EXTI_H_
#define _STM32F103X6_EXTI_H_
...
typedef struct {
	uint16_t EXTI_LineNumber;   	//select EXTI input line number
	...
	uint16_t GPIO_Pin;			//select GPIO pin to be used as EXTI pin
	//this parameter can be a value of  @ref GPIO_Pins_define @ MCU GPIO Driver

	uint8_t IVT_IRQNumber;			//select Interrupt Vector Table request number
	//this parameter can be a value of @ref IVT_IRQNumber @ MCU DEVICE HEADER

}EXTI_GPIO_Mapping_t;

typedef struct {
	EXTI_GPIO_Mapping_t EXTI_Pin;   	//select EXTI input pin
	//this parameter can be a value of   @ref EXTI_define

	uint8_t Trigger_state;		//select trigger state (edge trigger)
	//this parameter can be a value of

	uint8_t IRQ_EN;			//select pending request register state
	//this parameter can be a value of  @ref EXTI_IRQ_EN_define (INT_MASK + NVIC Enable/Disable)

	void (*P_IRQ_CallBack)(void);			//pointer to IRQ call back function
